...
void setup()
{
  ...
  smartthing.shieldSetLED(0, 1, 0); // flash green to start
  delay(500);
  ...
  // For ref purpose: here is the LED control sheme from ST library:
  ...
  //set up relays to control irrigation valves (pins 21 to 36)
  int i=1;
  while (i<(relays+1)) {
    relay[i]= 20+i;
...
void toggleOn() {
  queue[trafficCop]=2;
  if (isConfigPump) {
  smartthing.shieldSetLED(0, 0, 1); //Blue for running
