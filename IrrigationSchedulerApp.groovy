...
definition(
    name: "Irrigation Scheduler v3.0.5_P-A",
    namespace: "pagauvreau/smart_sprinkler_P-A",
    author: "matt@nichols.name and stan@dotson.info and pagt666@gmail.com",
    description: "Schedule sprinklers to run unless there is rain. Some tweaks and new features in development by pagauvreau",
    version: "3.0.5",
...
preferences {
	page(name: "schedulePage", title: "Create An Irrigation Schedule", nextPage: "sprinklerPage", uninstall: true) {
		...
		section {
            ...
            name: "wateringDates",
            type: "enum",
            title: "Water only on even or odd dates?",
            options: ['Even','Odd'],
			required: false)
        }
	...
	page(name: "weatherPage", title: "Virtual Weather Station Setup", install: true) {
        ...
        section("Zip code or Weather Station ID to check weather...") {
            input "zipcode", "text", title: "Enter zipcode or or pws:stationid", defaultValue: "pws:IQCMONTR126", required: false
        }
...
// 	  Next line is useful log statement for debugging why the smart app may not be triggering.
	log.info("${app.label} scheduler state: ${schedulerState}. Days since last watering: ${daysSince()}. Is watering day? ${isWateringDay()}. Is watering date (Odd/Even)? ${isWateringDate()}. Enought time? ${enoughTimeElapsed(schedulerState)} ")

    if ((isWateringDay() && isWateringDate() && enoughTimeElapsed(schedulerState) && schedulerState != "delay") || schedulerState == "expedite") {
...
def isWateringDate() {
    if(!wateringDates) return true
	//log.info "Watering is only done on ${wateringDates} dates."
    int today = Integer.parseInt(new Date().format("d", location.timeZone))
	//log.info "Today's date is ${today} and its modulo of 2 is ${today%2}"
    if (today%2 ==0 && wateringDates == "Even" || today%2 != 0 && wateringDates != "Even") { //== "Odd") {
        return true
    }
    log.info "Watering is only done on ${wateringDates} dates, so not today."
    return false
}
